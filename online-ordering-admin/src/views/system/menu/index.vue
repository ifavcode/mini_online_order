<template>
  <div class="app-container">
    ...
    <div class="mt-2">
      <p class="text-sm text-red-500 italic">注：路由名称不允许重复</p>
    </div>
      ...
      <el-table-column prop="icon" label="图标" align="center" width="100">
        <template #default="scope">
          <div class="flex justify-center">
            <IconifyIconOnline :icon="scope.row.icon"></IconifyIconOnline>
          </div>
        </template>
      </el-table-column>
      <el-table-column
          ...
          <el-col :span="12">
            <el-form-item prop="component">
              <template #label>
                <span>
                  <el-tooltip
                    content="访问的组件路径，如：`system/user/index`，默认在`views`目录下（已经存在的请不要修改）"
                    placement="top"
                  >
                    <el-icon>
                      <QuestionFilled />
...
function getList() {
  ...
  listMenu(queryParams.value).then(response => {
    menuList.value = handleTree(response.content, "id");
    menuList.value.sort((a, b) => a.orderNum - b.orderNum);
    loading.value = false;
  });
}
...
function getTreeselect() {
  ...
  listMenu().then(response => {
    ...
    menu.children = handleTree(response.content, "id");
